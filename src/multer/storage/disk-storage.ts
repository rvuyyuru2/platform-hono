...
const executeStorageHandler = async (
  file: File,
  req: HonoRequest,
  obj?: DiskStorageOptionHandler,
): Promise<string | null> => {
  if (typeof obj === 'function') {
    return await obj(file, req);
  }
...
export class DiskStorage implements Storage<DiskStorageFile, DiskStorageOptions> {
  ...
  public async handleFile(file: File, req: HonoRequest, fieldName: string): Promise<DiskStorageFile> {
  ...
  protected async getFilename(
    ...
    return (await executeStorageHandler(file, req, obj)) ?? (await getUniqueFilename(file.name));
  }
