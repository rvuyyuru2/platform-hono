...
> extends AbstractGraphQLDriver {
  ...
  async start(options: T): Promise<void> {
    ...
    if (platformName !== 'hono') {
      throw new Error('HonoGraphQLDriver is only compatible with the Hono platform');
    }
    ...
    app.use(path, async (ctx: Context) => {
      ...
      const httpGraphQLResponse = await server.executeHTTPGraphQLRequest({
        httpGraphQLRequest: {
          ...
          search: ctx.req.url.includes('?')
            ? ctx.req.url.slice(ctx.req.url.indexOf('?'))
            : '',
        },
        context: () =>
          contextFunction(ctx, {
  ...
  private httpHeadersToMap(headers: Headers): HeaderMap {
  ...
  private async parseBody(ctx: Context): Promise<Record<string, unknown>> {
    ...
    if (contentType.startsWith('application/json')) {
      return req.json().catch(() => this.logError(new Error('Invalid JSON body')));
    }
    ...
    // Return empty body for GET requests (introspection, persisted queries)
    return {};
  }

  private logError(e: unknown): never {
  ...
  private async parseFormURL(req: HonoRequest): Promise<Record<string, string>> {
